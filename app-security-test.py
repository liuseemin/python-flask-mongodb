...
#=====================================================        
# experiment on dynamic form control
...
class main_form(FlaskForm):
    title = StringField('Title', validators=[DataRequired()])
    group_form_item = FieldList(FormField(GroupForm), min_entries=1)
...
# handle group form addition
@app.route('/add_group', methods=['POST'])
def add_group():
    # Get the current count of groups to create unique IDs for each new group
    print(request.form)
    return "success"
    if request.method == 'POST':
        ...
        # Render the HTML for a new group of fields
        group_form = GroupForm(prefix=f'groups-{group_id}')  # Ensure unique field names
        group_html = render_template('group.html', group_form=group_form)

        # Return the new group HTML in a JSON response
        return jsonify({'new_group': group_html})
